  ...
  packages() {
    return this.rawText
      ...
      .map(pkg => {
        ...
        let name, version;
        let parsedPackageInfo = packageHeader.split('@');
        if (parsedPackageInfo.length > 2) {
          parsedPackageInfo = parsedPackageInfo
            .map(entry => entry.split(', '))
            .reduce((acc, entry) => acc.concat([entry[0]]), []);
        }
        parsedPackageInfo[parsedPackageInfo.length - 1] = parsedPackageInfo[
          parsedPackageInfo.length - 1
        ].substring(
          0,
          parsedPackageInfo[parsedPackageInfo.length - 1].length - 1,
        );
